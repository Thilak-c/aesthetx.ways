...
import { FiEdit, FiTrash2, FiEye, FiEyeOff, FiTrendingUp, FiClock, FiSearch } from "react-icons/fi";
...
export default function ProductDashboard() {
  ...
  const filteredProducts = useMemo(() => 
    products
      ...
      }), [products, search, sortBy, sortAsc, selectedCategory]);
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredProducts.map(p => (
              <tr key={p.itemId} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-2">{p.category || "Uncategorized"}</td>
                <td className="px-4 py-2 font-bold">₹{p.price.toLocaleString()}</td>
                <td className="px-4 py-2"><AnimatedNumber value={p.buys || 0} /></td>
                <td className="px-4 py-2 font-bold">₹<AnimatedNumber value={(p.buys || 0) * (p.price || 0)} /></td>
                ...
                <td className="px-4 py-2 flex gap-2">
                  ...
                  <button
                    ...
                    className="px-2 py-1 bg-gray-700 text-white rounded hover:bg-gray-900 text-sm flex items-center gap-1"
                  >
                    <FiTrash2 /> Delete
                  </button>
                  <button
                    onClick={() => handleToggleHidden(p)}
                    className={`px-2 py-1 rounded text-sm flex items-center gap-1 text-white ${
                      p.isHidden ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
                    }`}
                  >
                    {p.isHidden ? <><FiEye /> Unhide</> : <><FiEyeOff /> Hide</>}
                  </button>
                </td>
              </tr>
            ))}
