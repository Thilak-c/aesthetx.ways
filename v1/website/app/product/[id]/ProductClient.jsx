...
import { ArrowLeft, ShoppingBag, Plus, Minus, Check, ChevronLeft, ChevronRight, Heart, Share2 } from 'lucide-react';
...
export default function ProductClient({ params, initialProduct }) {
  ...
  const handleFavoriteToggle = (e) => {
    ...
    if (typeof window !== 'undefined' && product) {
      const wishlist = JSON.parse(localStorage.getItem('aw_wishlist') || '[]');
      let updatedWishlist;
      if (wishlist.includes(product.itemId)) {
        updatedWishlist = wishlist.filter(id => id !== product.itemId);
        setIsFavorited(false);
        setToastMessage("Removed from Wishlist");
        setShowToast(true);
        setTimeout(() => setShowToast(false), 3000);
      } else {
        updatedWishlist = [...wishlist, product.itemId];
        setIsFavorited(true);
    ...
    if (typeof window !== 'undefined' && product) {
      try {
        const shareData = {
          ...
          text: `Check out ${product.name || 'this product'} on Aesthetx Ways!`,
          url: window.location.href,
        };

        // Try to fetch image and convert to file for native sharing sheet preview
        if (product.mainImage && navigator.share && navigator.canShare) {
          try {
            const proxiedUrl = `/api/proxy-image?url=${encodeURIComponent(product.mainImage)}`;
            const response = await fetch(proxiedUrl);
            if (response.ok) {
              const blob = await response.blob();
              const contentType = blob.type || 'image/png';
              const extension = contentType.split('/')[1] || 'png';
              const file = new File([blob], `${product.itemId}.${extension}`, { type: contentType });
            ...
            console.error('Failed to share with image file:', err);
          }
        }
  ...
  useEffect(() => {
    if (cartCount > 0) {
      ...
      const timer2 = setTimeout(() => setPulse(false), 450);
      return () => {
        clearTimeout(timer1);
        clearTimeout(timer2);
      };
    ...
    <>
      <div className="flex flex-col flex-1 bg-white relative pb-28 animate-slide-up-fade">
        ...
        <div className="relative w-full aspect-[4/5] bg-zinc-50 border-b border-zinc-100 group overflow-hidden">
          ...
          {/* Glassy Share & Heart Floating Icon Buttons */}
          <div className="absolute bottom-4 right-4 z-30 flex flex-col gap-2.5">
            <button
              onClick={handleFavoriteToggle}
              className="w-9 h-9 rounded-full bg-white/70 hover:bg-white/90 border border-zinc-200/50 backdrop-blur-md flex items-center justify-center transition-all duration-300 active:scale-90 cursor-pointer shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
              aria-label="Wishlist Product"
            >
              <Heart className={`w-4 h-4 transition-transform duration-200 active:scale-125 ${isFavorited ? 'fill-red-500 text-red-500' : 'text-zinc-700'}`} />
            </button>
            <button
              onClick={handleShareClick}
              className="w-9 h-9 rounded-full bg-white/70 hover:bg-white/90 border border-zinc-200/50 backdrop-blur-md flex items-center justify-center text-zinc-700 hover:text-black transition-all duration-300 active:scale-90 cursor-pointer shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
      ...
      <div className="fixed bottom-11 left-0 right-0 z-40 bg-white border-t border-zinc-100 px-4 py-3 flex items-center justify-between max-w-[430px] mx-auto shadow-[0_-4px_12px_rgba(0,0,0,0.02)]">
        ...
        >
          <span>{addedToCart ? 'Go to Bag' : 'Add to Bag'}</span>
